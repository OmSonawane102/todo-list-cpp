...
- Feedback message when completing or deleting an ID that doesn't exist
- Guards against completing/deleting when the list is empty, with a clear message
- Validated numeric input — non-numeric entries are rejected and re-prompted instead of corrupting later reads
- Validated menu input — extra characters typed after a menu key are discarded, and unrecognized keys show an "Invalid Option" message
...
- A terminal that supports ANSI escape codes for colored output (see Known Limitations)
...
- If you type something other than a number when an ID is requested, the program tells you and asks again — it won't get stuck or misread later input.
- If you type an unrecognized menu key (or multiple characters), the extra characters are discarded and you'll see "Invalid Option..." instead of the app silently misbehaving.
- If you try to complete or delete a task while your list is empty, the app tells you there's nothing to act on instead of asking for an ID first.
- If you enter an ID that doesn't exist for c or d, the app tells you and waits for you to press Enter before returning to the menu.

## Known Limitations (For current version - v1.6.0)
...
**v1.6.0**
- Added a validIntInput() helper function that repeatedly prompts until a valid integer is entered, using std::cin.fail(), std::cin.clear(), and std::cin.ignore() to recover from bad input instead of leaving the stream in a broken state. Applied to both the complete and delete ID prompts.
- Flushed the rest of the input line immediately after reading the menu key, so typing multiple characters (e.g. "hello") no longer causes the menu to silently process each leftover character as a separate loop iteration.
- Re-enabled the previously commented-out "Invalid Option" branch for unrecognized menu keys.
- Added empty-list guards to both the complete and delete branches, so acting on an empty list gives an immediate, clear message instead of prompting for an ID first.
